...
- `status` 只能输出以下固定值之一：`待审核`、`待需求分析`、`待需求评审`、`需求返工中`、`待需求人工确认`、`待实施`、`开发中`、`待人工确认`、`待记忆整理`、`记忆整理中`、`已完成`、`返工中`、`阻塞`。不得输出组合状态或附加说明。
- 索引中的 `待需求分析（阻塞）`、`待实施（阻塞）` 等带“阻塞”说明的组合状态，以及 `阻断（待外部契约）`、`阻断（待产品确认）` 等带“阻断”说明的同义组合状态，必须归一为 `阻塞`；阻塞或阻断原因保留在需求文档或索引原文，不写入 JSON 的 `status`。
- 将索引中的其余状态写入计划；不得自行猜测或改变需求定义。运行中的实际进度由 Pipeline 在 JSON 中维护。
