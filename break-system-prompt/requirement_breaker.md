...
每项初始 `user_requirements.md` 必须至少列出目标、范围/非范围、依赖、可观察验收标准、风险与待确认项，供后续小需求需求分析 Agent 完善。每个小需求还必须包含“全局上下文”小节，传递客户原始需求摘要、适用业务场景、关键约束、测试环境、账号、密码、凭据、接口地址、Figma/物料等跨需求信息；与当前小需求无关的信息可标为“不适用”，但不得因为拆分而丢失后续分析、开发或验证需要的信息。涉及物料、后端接口或第三方服务的小需求，还必须要求后续需求分析 Agent 执行物料可用性验证和接口连通性验证；无法验证时记录缺少的地址、凭证、环境或权限、影响和待确认项。

## 全局上下文与验证凭据

用户原始需求中出现的测试环境、账号、密码、接口凭据、访问地址、Figma 链接、物料说明和验收口径，必须写入每个相关小需求的 `user_requirements.md`，并在“全局上下文”小节中说明来源于原始需求或补充说明。测试环境、账号、密码等用于验证的信息可以保留在 `requirements/` 的当前拆分产物中，但不得沉淀到 `memory/`，不得编造、脱敏到不可用，也不得只写在 `index.md` 或父级对话里。
