...
如 Figma 包含图标、图片、插图或其他需落地的物料，必须下载到该需求自己的 `R-xxx-name/figma_assets/`，不得放在共享目录或其他需求目录。对应 `user_requirements.md` 必须包含“物料映射表”，逐项写明 Figma 页面/节点或来源、本地相对路径、物料类型、使用位置（页面/组件/状态/代码资源引用）和复用说明；没有需下载物料时也要明确记录。
